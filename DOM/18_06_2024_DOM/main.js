...
//FUNCION QUE PINTA UNA LISTA DE PERSONAJES CON NAME DESCRIPTION E IMAGE
const paintCharacters = (characterList) => {
  //Recorremos los personajes y le indicamos la tarea a cada una de las vueltas
  for (const character of characterList) {
    //Por cada una de las vueltas....
    //Creamos un li vacio
    const li = document.createElement("li");
    //RELLENAMOS LOS LI CON IMG, H2 Y P
    const img = document.createElement("img");
    img.src = character.image;
    img.alt = character.name;
    //METEMOS EL IMG EN EL LI
    li.appendChild(img);
    const h2 = document.createElement("h2");
    h2.textContent = character.name;
    //METEMOS EL H2 EN EL LI
    li.appendChild(h2);
    const p = document.createElement("p");
    p.textContent = character.description;
    //METEMOS EL P EN EL LI
    li.appendChild(p);
    //EL LI COMPLETO VA AL CONTENEDOR
    galleryContainer.appendChild(li);
  }
};

//Vamos añadir la función reverse al botón
const reverseBtn = document.querySelector("#reverse-btn");
reverseBtn.addEventListener("click", () => {
  //Le damos la vuelta a characters
  characters.reverse();
  //Antes de pintar, limpiamos el contenedor de ul
  galleryContainer.innerHTML = "";
  //Los volvemos a pintar
  paintCharacters(characters);
});

//Vamos a añadir la función de cambio de tema
const themeBtn = document.querySelector("#theme-btn");
themeBtn.addEventListener("click", () => {
  document.body.classList.toggle("light");
});
//POR DEFECTO PINTAMOS CHARACTERS
paintCharacters(characters);
